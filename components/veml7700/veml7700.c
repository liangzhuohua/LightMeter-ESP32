...
#define VEML7700_RESOLUTION_800MS_IT_GAIN_2     (42)
...
esp_err_t veml7700_get_config(i2c_dev_t *dev, veml7700_config_t *config)
{
    ...
    config->persistence_protect = (config_data & VEML7700_PERSISTENCE_PROTECTION_MASK)
                               >> VEML7700_PERSISTENCE_PROTECTION_SHIFT;
    config->interrupt_enable = (config_data & VEML7700_INTERRUPT_ENABLE_MASK)
                               >> VEML7700_INTERRUPT_ENABLE_SHIFT;
    config->shutdown = (config_data & VEML7700_SHUTDOWN_MASK)
