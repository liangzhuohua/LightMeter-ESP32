...
esp_err_t hw_sd_write_file(const char *path, char *data)
{
...
esp_err_t hw_sd_read_file(const char *path)
{
...
esp_err_t hw_sd_init(void) {
    ...
    ret = hw_sd_write_file(file_hello, data);
    if (ret != ESP_OK) {
        return ret;
    }

    // 打开txt文件，并读出文件中的内容
    ret = hw_sd_read_file(file_hello);
    if (ret != ESP_OK) {
        return ret;
    }
